var search             = require ('./search')
    ...
    , prepareFileName  = require ('../utils/prepareFileName')
    , slug             = require ('slug');
...
function installFontsFromConfig () {
  InitConfigEngine.read(function (error, data) {
    _.each(JSON.parse(data).fonts, function (fontName) {
      remoteInstall(prepareFontName(fontName), true);
    });
  });
};
...
function remoteInstall (font, installFromConfig) {
  var buffer = '';
  InitConfigEngine.read(function (error, data) {
    var scope = this;
    var config = {};
    var extension = 'css'; //TODO in te future will be used for sass, less, ...
    if (data) {
    ...
    if (!isFontAlreadyInstalled(config, font, extension, installFromConfig)) {
...
function isFontAlreadyInstalled(config, fontName, extension, installFromConfig) {

  var finalDestination = process.cwd() + config.directory;
  var fileName = prepareFileName(fontName, extension);

  if (StreamEngine.fileExists(finalDestination + '/' + fileName)) {
    return true;
  }
  if (!installFromConfig && _.contains(config.fonts, fontName)) {
    return true;
  }
